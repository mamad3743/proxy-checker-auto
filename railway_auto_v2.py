...
---------------------------------------------------
Run it, complete the Railway login in your browser (a pairing
code/link opens automatically), pick a project from a menu, let
Railway's own picker choose environment/service, and the rest is
automatic: create a TCP proxy, resolve its edge hostname to an IP
(the hostname itself does not answer ICMP ping), save it into
...
def main():
    ...
    banner("Step 1 — Login")
    info("A browser window/pairing code will open — approve it there.")
    env = os.environ.copy()
