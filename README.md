...
1. **لوگو + Step 1 — Login:** خودش دستور `railway login` رو اجرا می‌کنه — یه کد/لینک برات باز می‌شه، تو مرورگر تأییدش می‌کنی، همین. (قبلاً از طریق توکن دستی بود ولی چون بعضی وقتا Railway با توکن‌های Account خطای عجیب می‌داد، حالا از همون روش لاگین مرورگری استاندارد استفاده می‌کنه که مطمئن‌تره.)
   ...
   اگه شبکه/ISP‌ات جلوی `backboard.railway.com` رو گرفته باشه (بلاک سطح Cloudflare، نه باگ اسکریپت)، خودش تشخیص می‌ده و ازت می‌پرسه پراکسی داری؟ — اگه دادی فقط همون اجرا رو از پراکسی رد می‌کنه (چیزی مشترک/داخلی ساخته نمی‌شه)، اگه نداشتی هم چندتا پیشنهاد (VPN، دیتای موبایل، DNS 1.1.1.1) نشون می‌ده.
